...
const makeGraphAuthService = Effect.gen(function* () {
  ...
  const getGraphClient = Effect.gen(function* () {
    ...
    return yield* Effect.try({
      try: () => {
        ...
        new GraphClientInitializationError({
  ...
  const validateToken = Effect.gen(function* () {
    ...
    const expiry = yield* Ref.get(expiryRef);
    return (
      token !== null && expiry !== null && expiry > new Date(Date.now() + 60000)
    );
  });
...
export class GraphAuthService extends Effect.Service<GraphAuthService>()(
  'GraphAuthService',
  {
