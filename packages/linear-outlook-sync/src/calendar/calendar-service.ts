...
import {GraphAuthService, GraphAuthServiceLive} from "../auth/graph-auth-service"
...
const makeCalendarService = Effect.gen(function* () {
    ...
    const listCalendars = (email: string) => Effect.gen(function* () {
        ...
        const parsedResponse = yield* Schema.decodeUnknown(CalendarListResponse)(response).pipe(
            Effect.mapError(error => parseGraphResponseError(endpoint, response, error.message))
        )
     ...
     */
    const listEvents = (options: { email: string, start?: DateTime, end?: DateTime }) => Effect.gen(function* () {
        const graphClient = yield* authService.getGraphClient
        ...
        const response = yield* Effect.tryPromise({
            try: () => graphClient.api(endpoint)
                .query({startDateTime: startDate.toISODate()!, endDateTime: endDate.toISODate()!})
                .get(),
            catch: (error) => handleGraphError(error, endpoint)
        })

        const parsedResponse = yield* Schema.decodeUnknown(EventListResponse)(response).pipe(
            Effect.mapError(error => parseGraphResponseError(endpoint, response, error.message))
        )
...
export class CalendarService extends Effect.Service<CalendarService>()("CalendarService", {
